...
function MedicHistory() {
  ...
  function getAcceptedRequests() {
    fetch(`http://localhost:8080/doctor/viewRecipes/${sessionStorage.getItem('id')}?status=APPROVED`, {
      method: "GET",
      headers: { "content-type": "application/json", Authorization: `Bearer ${sessionStorage.getItem('token')}` },
    })
      .then((response) => {
  ...
  function getDeclinedRequests() {
    fetch(`http://localhost:8080/doctor/viewRecipes/${sessionStorage.getItem('id')}?status=DECLINED`, {
      method: "GET",
      headers: { "content-type": "application/json", Authorization: `Bearer ${sessionStorage.getItem('token')}` },
    })
      .then((response) => {
...
<p style={cardTextStyle}>Pharmacy Name: {request.pharmacyName}</p>
      ...
      <Panel header='Show accepted requests'>
...
<p style={cardTextStyle}>Pharmacy Name: {request.pharmacyName}</p>
      ...
      <Panel header='Show declined requests'>
